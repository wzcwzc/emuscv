...
scrntype_t* EMU::get_screen_ptr()
{
	return osd->get_vm_screen_ptr();
}

scrntype_t* EMU::get_line_ptr(int y)
{
	return osd->get_vm_line_ptr(y);
}

scrntype_t* EMU::get_pixel_ptr(int x, int y)
{
	return osd->get_vm_pixel_ptr(x, y);
}
...
bool EMU::is_bios_found()
{
	return vm->is_bios_found();
}

bool EMU::is_bios_present()
{
	return vm->is_bios_present();
}

bool EMU::is_bios_ok()
{
	return vm->is_bios_ok();
}

void EMU::set_cpu_clocks(uint32_t clocks)
{
	vm->set_cpu_clocks(clocks);
}
